...
function checkResponse {
    NOMATCH=`echo ${1} |grep NO_MATCHING_DATA `
    AUTH_ERROR=`echo ${1}|grep ":401," `
    NF_ERROR=`echo ${1}|grep ":404," `
    BAD_METHOD=`echo ${1}|grep ":405," `
    if [ -n "${NOMATCH}" ]; then
       echo ${NOMATCH}
       exit 0
       ...
       exit -1
    elif [ -n  "${BAD_METHOD}" ]; then
       echo Incorrect curl verb was used in your request
       echo ${BAD_METHOD}
       exit -1
    fi
}
...
# don't delete data!
echo Acknowledging the nofari for ${date} will make the following data no longer accessible:
echo ${NOARFI}
echo -e "\nThis data is persisted in ${outputFile}"
echo -e "\nAre you sure? 'Y' to continue, anything else to quit\n"

read c

if [ $c == "Y" ]; then
   # code is still not asking for a DELETE.
    DELETE=$( \
        curl -s \
        ${CURL_DEBUG} \
        -k -X DONT_DELETE \
   ...
   echo "Acknowlgement averted with '$c' response"
fi
