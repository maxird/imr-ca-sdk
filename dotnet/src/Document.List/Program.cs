    ...
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            try
            {
                ...
                if (args.Length != 1) {
                    Console.WriteLine("usage: list <caseNumber>");
                    Environment.Exit(1);
                }
                ...
                var result = caseService.GetDocuments(caseNumber);

                // shape the results to list the renderings which is what
                // we need to retrieve a document
                //
                foreach (var item in result.results)
                {
                    var name = item.name;
                    foreach (var rendering in item.renderings)
                    {
                        var identifier = rendering.identifier;
                        var mimeType = rendering.type;

                        Console.WriteLine($"{identifier}\t{mimeType}\t\"{name}\"");
                    }
                }
            }
